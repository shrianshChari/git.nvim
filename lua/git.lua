...
local function config_commands()
  ...
  vim.api.nvim_create_user_command("G", 'lua require("git.cmd").cmd(<f-args>)', {
      bang = true,
      nargs = '*',
    })

  vim.api.nvim_create_user_command("GDiff", 'lua require("git.diff").open(<f-args>)', {
      bang = true,
      nargs = '*',
    })

  vim.api.nvim_create_user_command("GDiffClose", 'lua require("git.diff").close()', {
      bang = true,
    })
